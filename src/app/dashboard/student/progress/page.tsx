...
function StatCard({ label, value, sub, accent }: {
  ...
  return (
    ...
    >
      <div className="absolute inset-x-0 top-0 h-[3px] rounded-t-2xl" style={{ background: accent ?? "rgb(var(--primary))" }} />
      <div className="text-[11px] font-semibold uppercase tracking-widest text-[rgb(var(--muted2))] mt-1">{label}</div>
      <div className="text-2xl font-bold text-[rgb(var(--fg))] leading-tight">{value}</div>
      {sub && <div className="text-[11px] text-[rgb(var(--muted2))] leading-snug">{sub}</div>}
    </div>
  );
}
  ...
  return (
    <div className="mt-2 space-y-1">
      <div className="flex items-center justify-between text-[10px] text-[rgb(var(--muted2))]">
        ...
        <span className={gain >= 0 ? "text-emerald-500 font-semibold" : "text-rose-500 font-semibold"}>
      ...
      <div className="h-1.5 rounded-full bg-[rgb(var(--border))] overflow-hidden">
        <motion.div
          ...
          className={["h-full rounded-full", gain >= 0 ? "bg-emerald-500" : "bg-rose-400"].join(" ")}
        />
      </div>
    </div>
  );
...
export default function StudentProgressPage() {
  ...
  const tab: TabKey = rawTab === "topics" ? "topics" : rawTab === "history" ? "history" : "overview";
  ...
  const { data, isLoading: loading, error } = useSWR<ApiResp>(
    apiUrl,
    fetcher,
    { revalidateOnFocus: false, dedupingInterval: 15_000 }
  );

  const msg = (!loading && error) ? "Unable to load progress." :
    (!loading && data && !data.ok) ? (data.message ?? "Unable to load progress.") : null;
    ...
    <div className="space-y-5">
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-5">
          <div>
            <h1 className="text-xl font-bold tracking-tight text-[rgb(var(--fg))]">Progress</h1>
            <p className="mt-0.5 text-[12px] text-[rgb(var(--muted2))]">Sessions · Topics · Confidence trends</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 rounded-full border border-[rgb(var(--border))] bg-[rgb(var(--card2))] p-1">
              <TabButton active={tab === "overview"} onClick={() => setTab("overview")} icon={
              ...
              <TabButton active={tab === "topics"} onClick={() => setTab("topics")} icon={
                <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                  <path d="M1 4h12M1 8h8M1 12h5" />
                </svg>
              }>Topics</TabButton>
              <TabButton active={tab === "history"} onClick={() => setTab("history")} icon={
                <svg width="10" height="10" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
      ...
      {/* Error message */}
      {msg && (
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/05 p-4 text-sm text-rose-500">{msg}</div>
      )}
            ...
            {tab === "overview" && (
              ...
              >
                <div className="grid gap-3 sm:grid-cols-3">
                  <StatCard label="Total Sessions" value={overview?.totals.totalSessions ?? 0} accent="rgb(99,102,241)" />
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] p-5 space-y-3">
                    ...
                    {analytics?.mostImprovedSubject ? (
                      <>
                        <div>
                          <div className="text-sm font-bold text-[rgb(var(--fg))]">{analytics.mostImprovedSubject.code}</div>
                          <div className="text-xs text-[rgb(var(--muted2))]">{analytics.mostImprovedSubject.title}</div>
                        </div>
                        <div className="flex flex-wrap gap-1.5">
                          <Pill green>+{analytics.mostImprovedSubject.avgConfGain.toFixed(2)} avg gain</Pill>
                          <Pill>{analytics.mostImprovedSubject.totalSessions} sessions</Pill>
                        </div>
                        <button
                          type="button"
                          onClick={() => setSubject(analytics.mostImprovedSubject!.subjectId, "topics")}
                        ...
                        {analytics!.confidenceTrend.map((x, idx) => (
                          ...
                          >
                            <div className="text-[11px] text-[rgb(var(--muted2))]">
                              <span className="font-semibold text-[rgb(var(--fg))]">{x.subjectCode}</span>{" "}
                            ...
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm font-bold text-[rgb(var(--fg))]">{s.code}</span>
                                  <span className="text-xs text-[rgb(var(--muted2))] truncate">— {s.title}</span>
                                </div>
                              ...
                              <div className="flex flex-wrap gap-1.5 items-center">
                                ...
                                <svg className="text-[rgb(var(--muted2))] group-hover:text-[rgb(var(--primary))] transition-colors" width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="min-w-0 flex items-start gap-3">
                            ...
                            <div>
                              <div className="text-sm font-bold text-[rgb(var(--fg))]">{t.topicName}</div>
                              <div className="mt-0.5 text-[11px] text-[rgb(var(--muted2))]">{t.subjectCode} — {t.subjectTitle}</div>
                            </div>
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            <Pill>{t.timesCovered} time{t.timesCovered !== 1 ? "s" : ""}</Pill>
                            <Pill>last: {t.lastCoveredAt ? new Date(t.lastCoveredAt).toLocaleDateString() : "—"}</Pill>
                          </div>
                        </div>
                        <div className="mt-3 h-1 rounded-full bg-[rgb(var(--border))] overflow-hidden">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${Math.min(100, (t.timesCovered / (topics!.topTopics[0]?.timesCovered || 1)) * 100)}%` }}
                      ...
                      >
                        <div className="flex">
                          ...
                          <div className="flex-1 p-4 space-y-3">
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              <div className="min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm font-bold text-[rgb(var(--fg))]">{s.subject.code}</span>
                                  <span className="text-xs text-[rgb(var(--muted2))] truncate">— {s.subject.title}</span>
                                </div>
                                <div className="mt-1 text-[11px] text-[rgb(var(--muted2))] pl-8">
                            ...
                            {s.completion && <ConfBar before={s.completion.confidenceBefore} after={s.completion.confidenceAfter} />}

                            <div className="space-y-1">
                              <div className="text-[10px] font-semibold uppercase tracking-widest text-[rgb(var(--muted2))]">Summary</div>
                            ...
                            {s.completion?.topics?.length ? (
                              <div className="flex flex-wrap gap-1.5">
                                {s.completion.topics.map((t) => <Pill key={t}>{t}</Pill>)}
                              </div>
                            ) : null}
