  ...
  return (
    <>
      <style>{`
        ...
        .rw-hgroup { display: flex; align-items: center; gap: 13px; }
        .rw-icon-wrap { position: relative; display: flex; align-items: center; justify-content: center; width: 46px; height: 46px; border-radius: 15px; border: 1px solid rgb(var(--border)); background: rgb(var(--card)); flex-shrink: 0; animation: rw-float 3.8s ease-in-out infinite; }
        .rw-icon-halo { position: absolute; inset: -8px; border-radius: 20px; background: radial-gradient(circle, rgb(var(--primary)/.30) 0%, transparent 70%); filter: blur(10px); }
        .rw-icon-wrap svg { position:relative; z-index:1; color:rgb(var(--primary)); width:20px; height:20px; }
        .rw-h1 { font-size: 1.1875rem; font-weight: 700; color: rgb(var(--fg)); letter-spacing: -0.025em; line-height: 1.2; }
        .rw-sub { font-size: 0.72rem; color: rgb(var(--muted)); margin-top: 1px; }
        .rw-wallet { display: inline-flex; align-items: center; gap: 7px; padding: 7px 16px; border-radius: 100px; border: 1px solid rgb(var(--border)); background: rgb(var(--card)); font-size: .85rem; position: relative; overflow: hidden; transition: box-shadow .2s; }
        .rw-wallet:hover { box-shadow: 0 0 0 2px rgb(var(--primary)/.18); }
        .rw-wallet-shine { position: absolute; inset:0; background: linear-gradient(105deg, transparent 38%, rgb(var(--primary)/.09) 50%, transparent 62%); background-size: 200% 100%; animation: rw-shimmer 3.2s linear infinite; }
        ...
        .rw-error { border-radius: 13px; border: 1px solid rgb(239 68 68 /.28); background: rgb(239 68 68 /.07); padding: 11px 15px; font-size: .8rem; color: rgb(252 165 165); animation: rw-fade-up .3s ease both; }
        .rw-buffbar { border-radius: 18px; border: 1px solid rgb(var(--border)); background: rgb(var(--card)); padding: 13px 16px; display: flex; flex-direction: column; gap: 9px; position: relative; overflow: hidden; animation: rw-fade-up .4s ease both; }
        .rw-buffbar-bg { position: absolute; inset:0; background: linear-gradient(135deg, rgb(var(--primary)/.07) 0%, transparent 55%); pointer-events:none; }
        .rw-buff-row { display: flex; align-items: center; justify-content: space-between; gap: 8px; position: relative; }
        .rw-buff-name { display: flex; align-items: center; gap: 8px; font-size: .84rem; font-weight: 600; color: rgb(var(--fg)); }
        .rw-dot { width: 7px; height: 7px; border-radius: 50%; background: rgb(var(--primary)); position: relative; flex-shrink: 0; }
        .rw-dot::after { content:""; position:absolute; inset:-4px; border-radius:50%; border:2px solid rgb(var(--primary)/.4); animation: rw-pulse-ring 2s ease-out infinite; }
        .rw-cd-seg { font-size: .78rem; font-weight: 700; color: rgb(var(--fg)); font-variant-numeric: tabular-nums; display: inline-flex; align-items: baseline; gap: 1px; }
        .rw-cd-u { font-size: .62rem; font-weight: 500; color: rgb(var(--muted)); }
        .rw-cd-sep { font-size: .78rem; font-weight: 700; color: rgb(var(--muted)); line-height:1; margin:0 1px; }
        .rw-grid { display: grid; gap: 16px; grid-template-columns: 1fr; }
        ...
        .rw-card { position: relative; border-radius: 20px; border: 1px solid rgb(var(--border)); background: rgb(var(--card)); padding: 19px; display: flex; flex-direction: column; overflow: hidden; transition: border-color .25s, box-shadow .25s, transform .2s; animation: rw-fade-up .5s ease both; }
        .rw-card:hover { transform: translateY(-3px) scale(1.005); border-color: rgb(var(--primary)/.35); box-shadow: 0 14px 40px -8px rgb(var(--shadow)/.16), 0 0 0 1px rgb(var(--primary)/.08); }
        .rw-card--active { box-shadow: 0 0 0 1px rgb(var(--primary)/.2), inset 0 0 24px rgb(var(--primary)/.04); }
        .rw-card:nth-child(1){animation-delay:.04s} .rw-card:nth-child(2){animation-delay:.09s} .rw-card:nth-child(3){animation-delay:.14s} .rw-card:nth-child(4){animation-delay:.19s} .rw-card:nth-child(5){animation-delay:.24s} .rw-card:nth-child(6){animation-delay:.29s}
        .rw-card-glow { position: absolute; top: -28px; right: -28px; width: 90px; height: 90px; border-radius: 50%; filter: blur(28px); opacity: 0; transition: opacity .3s ease; pointer-events: none; }
        .rw-card:hover .rw-card-glow { opacity:1; }
        .rw-icon-badge { display: flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 11px; border: 1px solid rgb(var(--border)); background: rgb(var(--card)); flex-shrink: 0; transition: transform .2s ease; }
        .rw-card:hover .rw-icon-badge { transform: scale(1.1) rotate(-5deg); }
        .rw-card-hdr { display: flex; align-items: flex-start; justify-content: space-between; gap: 10px; position: relative; }
        .rw-card-title-row { display: flex; align-items: flex-start; gap: 10px; }
        .rw-card-name { font-size: .9rem; font-weight: 650; color: rgb(var(--fg)); letter-spacing: -.015em; line-height: 1.25; margin-top: 3px; }
        .rw-cost { display: inline-flex; align-items: center; gap: 4px; border-radius: 9px; border: 1px solid rgb(var(--border)); background: rgb(var(--card2, var(--card))); padding: 3px 9px; font-size: .71rem; font-weight: 700; color: rgb(var(--fg)); white-space: nowrap; flex-shrink: 0; }
        .rw-cost-dot { width: 5px; height: 5px; border-radius: 50%; background: rgb(var(--primary)); }
        .rw-card-desc { font-size: .78rem; color: rgb(var(--muted)); line-height: 1.6; margin-top: 10px; position: relative; }
        .rw-meta { display: flex; align-items: center; justify-content: space-between; margin-top: 13px; font-size: .7rem; color: rgb(var(--muted)); position: relative; }
        .rw-meta-left { display:flex; align-items:center; gap:4px; }
        .rw-stock { display: inline-flex; align-items: center; gap: 3px; padding: 2px 7px; border-radius: 7px; font-size: .67rem; font-weight: 600; border: 1px solid rgb(var(--border)); background: rgb(var(--card2,var(--card))); }
        .rw-active-row { display: flex; align-items: center; justify-content: space-between; margin-top: 11px; padding: 7px 11px; border-radius: 11px; border: 1px solid rgb(var(--primary)/.28); background: rgb(var(--primary)/.07); position: relative; overflow: hidden; }
        .rw-active-shine { position: absolute; inset:0; background: linear-gradient(90deg, transparent, rgb(var(--primary)/.1), transparent); background-size:200% 100%; animation: rw-shimmer 2.5s linear infinite; }
        ...
        .rw-btn--off { background: rgb(var(--card2,var(--card))); border: 1px solid rgb(var(--border)); color: rgb(var(--muted2,var(--muted))); opacity: .55; cursor: not-allowed; }
        .rw-btn--active-state { background: rgb(var(--primary)/.09); border: 1px solid rgb(var(--primary)/.3); color: rgb(var(--primary)); cursor: not-allowed; }
        .rw-btn--success { background: rgb(34 197 94/.14); border: 1px solid rgb(34 197 94/.38); color: rgb(74 222 128); }
        .rw-skel { border-radius: 10px; background-image: linear-gradient(90deg, rgb(var(--border)) 0%, rgb(var(--card)) 50%, rgb(var(--border)) 100%); background-size: 200% 100%; animation: rw-shimmer 1.6s linear infinite; }
      `}</style>
