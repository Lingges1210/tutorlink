...
import {
  ArrowLeft, BookOpen, GraduationCap, FileText, Sparkles,
  ChevronRight, X, Upload, CheckCircle2, Clock, XCircle,
} from "lucide-react";
      ...
      {STEPS.map((s, i) => {
        ...
        return (
          <div key={i} className="flex items-center flex-1 last:flex-none">
            ...
            {i < STEPS.length - 1 && (
              <div className={["flex-1 h-px mx-1 transition-all duration-500", done ? "bg-[rgb(var(--primary)/0.4)]" : "bg-[rgb(var(--border))]"].join(" ")} />
            )}
          </div>
        );
      })}
...
function FloatInput({ label, value, onChange, placeholder, type = "text", step }: {
  ...
  return (
    <div className="relative">
      ...
      <label className={[
        ...
        (focused || filled) ? "top-2 text-[10px] text-[rgb(var(--primary))]" : "top-4 text-xs text-[rgb(var(--muted))]",
      ].join(" ")}>
        {label}
      </label>
    </div>
...
export default function ApplyTutorPage() {
  ...
  const existingStatus: AppStatus | null = useMemo(() => {
    if (!existingData?.success || !existingData?.application) return null;
    const raw = String(existingData.application.status ?? "").toUpperCase();
    return raw === "PENDING" || raw === "APPROVED" || raw === "REJECTED" ? (raw as AppStatus) : null;
  }, [existingData]);

  const rejectionReason: string | null = existingData?.application?.rejectionReason ?? null;

  // Pre-fill form on rejection
  useEffect(() => {
    if (existingStatus === "REJECTED" && existingData?.application) {
  ...
  function formatSubjectStrict(raw: string) {
    ...
    const title = titleRaw.toLowerCase().split(/\s+/).filter(Boolean).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
    return `${code} : ${title}`;
  }

  function addSubject(raw: string) {
    const formatted = formatSubjectStrict(raw);
    if (!formatted) { setSubjectError("Use format: CPT112 : Discrete Structures"); return; }
    setSubjects((prev) => {
  ...
  if (existingStatus === "APPROVED") return (
    <div className="max-w-xl space-y-5">
      ...
      />
      <Link href="/dashboard/tutor" className="inline-flex items-center gap-2 rounded-xl bg-[rgb(var(--primary))] px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-[rgb(var(--primary)/0.3)] hover:opacity-90 transition-opacity">
    ...
    <div className="max-w-xl space-y-6">
      ...
      {existingStatus === "REJECTED" && (
        <div className="rounded-2xl border border-rose-500/25 bg-rose-500/8 p-4 space-y-2">
          ...
          <p className="text-xs text-[rgb(var(--muted))] leading-relaxed">You may edit your details below and resubmit.</p>
          {rejectionReason?.trim() && (
            <div className="mt-1 rounded-xl border border-rose-500/20 bg-rose-500/8 px-3 py-2 text-xs text-[rgb(var(--fg))] leading-relaxed">
              <span className="font-semibold text-rose-500">Reason: </span>{rejectionReason}
            </div>
          )}
        </div>
      )}
      ...
      <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card)/0.6)] backdrop-blur-sm overflow-hidden">
        ...
        <div className="p-5 space-y-6">
          ...
          {/* SUBJECTS */}
          <Section step={1} icon={<BookOpen size={13} />} title="Subjects" hint="Add all subjects you'd like to tutor" done={subjects.length > 0}>
            {subjects.length > 0 && (
              <div className="mb-3 flex flex-wrap gap-2">
                {subjects.map((s) => {
                  const [code, title] = s.split(" : ");
                  return (
                    <span key={s} className="group inline-flex items-center gap-2 rounded-full border border-[rgb(var(--primary)/0.22)] bg-gradient-to-r from-[rgb(var(--primary)/0.15)] to-[rgb(var(--primary)/0.07)] px-3 py-1 text-[11px] font-medium text-[rgb(var(--fg))] transition-all hover:scale-[1.02] hover:shadow-md hover:shadow-[rgb(var(--primary)/0.12)]">
                      <span className="font-bold text-[rgb(var(--primary))]">{code}</span>
                      {title && <span className="opacity-70">{title}</span>}
                      <button type="button" onClick={() => removeSubject(s)} className="ml-0.5 flex h-4 w-4 items-center justify-center rounded-full bg-[rgb(var(--primary)/0.12)] text-[rgb(var(--primary))] transition group-hover:bg-rose-500 group-hover:text-white">
                        <X size={9} />
                      </button>
                    </span>
                  );
                  ...
                  <div style={{ maxHeight: 176, overflowY: "auto", overscrollBehavior: "contain" }}>
                    {filteredSuggestions.length > 0 ? filteredSuggestions.map((raw) => {
                      ...
                      return (
                        ...
                        >
                          <span style={{ minWidth: 80, textAlign: "center", borderRadius: 8, border: "1px solid rgb(var(--primary) / 0.25)", background: "rgb(var(--primary) / 0.1)", padding: "4px 10px", fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", color: "rgb(var(--primary))", flexShrink: 0 }}>
                            {(codeRaw ?? "").trim()}
                          </span>
          ...
          {/* CGPA */}
          <Section step={2} icon={<GraduationCap size={13} />} title="Academic Standing" hint="Your current cumulative GPA" done={cgpaNumber !== null}>
            <FloatInput label="CGPA" value={cgpa} onChange={setCgpa} placeholder="e.g. 3.80" type="number" step="0.01" />
            {cgpaNumber !== null && (
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold text-[rgb(var(--primary))]">{cgpaNumber.toFixed(2)} / 4.00</span>
              </div>
            )}
          </Section>
          ...
          {/* TRANSCRIPT */}
          <Section step={3} icon={<FileText size={13} />} title="Academic Transcript" hint="PDF, PNG, or JPG — max 8 MB" done={!!(transcriptPath || transcriptFile)}>
            <label
              onDragOver={(e) => { e.preventDefault(); setDragOver(true); }}
              onDragLeave={() => setDragOver(false)}
              onDrop={(e) => { e.preventDefault(); setDragOver(false); const file = e.dataTransfer.files?.[0] ?? null; if (file) onPickTranscript(file); }}
              className={[
                ...
                dragOver ? "border-[rgb(var(--primary))] bg-[rgb(var(--primary)/0.08)] scale-[1.01]"
                  : transcriptFile || transcriptPath ? "border-[rgb(var(--primary)/0.4)] bg-[rgb(var(--primary)/0.05)]"
                  : "border-[rgb(var(--border))] hover:border-[rgb(var(--primary)/0.5)] hover:bg-[rgb(var(--primary)/0.03)]",
              ].join(" ")}
            >
              <input type="file" accept=".pdf,image/png,image/jpeg,image/jpg" className="sr-only" onChange={(e) => onPickTranscript(e.target.files?.[0] ?? null)} />
              {transcriptFile || transcriptPath ? (
                <>
                  ...
                  <span className="text-[10px] text-[rgb(var(--primary))] font-medium">Click to replace</span>
                </>
              ) : (
                <>
                  ...
                  <p className="text-xs font-semibold text-[rgb(var(--fg))]">Drop your transcript here</p>
                  <p className="text-[10px] text-[rgb(var(--muted))]">or click to browse · PDF / PNG / JPG</p>
                </>
              )}
              {uploadingTranscript && <span className="text-[10px] text-amber-500 animate-pulse font-semibold">Uploading…</span>}
            </label>
          </Section>
          ...
          <div className="space-y-3 pt-1">
            <button
              type="button"
              disabled={loading || uploadingTranscript || subjects.length === 0 || cgpaNumber === null || (!transcriptPath && !transcriptFile)}
              onClick={submit}
            ...
            <Link href="/dashboard/student" className="flex items-center justify-center gap-1.5 text-xs text-[rgb(var(--muted))] hover:text-[rgb(var(--fg))] transition-colors">
...
function PageHeader() {
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        <h1 className="text-lg font-bold tracking-tight text-[rgb(var(--fg))]">Apply as Tutor</h1>
        <p className="mt-0.5 text-xs text-[rgb(var(--muted))]">Complete all three steps · Admin reviews within 1–2 days</p>
      </div>
    </div>
  );
}
...
function StatusCard({ icon, color, title, body }: {
  icon: React.ReactNode; color: "amber" | "emerald" | "rose"; title: string; body: string;
}) {
  const palette = {
  ...
  return (
    <div className={`rounded-2xl border ${palette.border} ${palette.bg} p-4 space-y-1.5`}>
      <div className="flex items-center gap-2.5">{icon}<span className="text-sm font-bold text-[rgb(var(--fg))]">{title}</span></div>
...
function Section({ step, icon, title, hint, done, children }: {
  step: number; icon: React.ReactNode; title: string; hint: string; done: boolean; children: React.ReactNode;
}) {
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2.5">
        <div className={["flex h-6 w-6 shrink-0 items-center justify-center rounded-lg text-[10px] font-bold transition-all duration-300", done ? "bg-[rgb(var(--primary))] text-white shadow-sm shadow-[rgb(var(--primary)/0.4)]" : "bg-[rgb(var(--border))] text-[rgb(var(--muted))]"].join(" ")}>
          {done ? "✓" : step}
        </div>
