...
function generateCandidateStartsForDay(args: {
  ...
  if (!dayAvail || dayAvail.off || !Array.isArray(dayAvail.slots) || dayAvail.slots.length === 0) {
    return [] as Date[];
  }
...
function hasClash(existing: { scheduledAt: Date; endsAt: Date | null; durationMin: number }[], start: Date, end: Date) {
  const aStart = start.getTime();
  const aEnd = end.getTime();

  return existing.some((x) => {
    const bStart = x.scheduledAt.getTime();
    const bEnd = (x.endsAt ? x.endsAt.getTime() : bStart + (x.durationMin ?? 60) * 60_000);
...
export async function GET(_req: Request, ctx: { params: Promise<{ subjectId: string }> }) {
  ...
  const durationMin = Math.min(Math.max(parseInt(durationStr ?? "60", 10) || 60, 15), 180);
  ...
  const activeSessions = await prisma.session.findMany({
    where: {
      ...
      // endsAt might be null, still ok we calculate
    },
    select: {
      tutorId: true,
      scheduledAt: true,
  ...
  const sessionsByTutor = new Map<string, { scheduledAt: Date; endsAt: Date | null; durationMin: number }[]>();
  ...
  const map = new Map<string, { start: string; end: string; count: number; tutorIds: string[] }>();
  ...
  for (const tutorId of finalTutorIds) {
    ...
    for (let i = 0; i < days; i++) {
      ...
      for (const st of starts) {
        ...
        if (!cur) {
          map.set(startIso, { start: startIso, end: endIso, count: 1, tutorIds: [tutorId] });
        } else {
          cur.count += 1;
  ...
  const items = Array.from(map.values()).sort((a, b) => +new Date(a.start) - +new Date(b.start));
