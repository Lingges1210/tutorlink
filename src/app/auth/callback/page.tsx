    ...
    if (code) {
      supabase.auth.exchangeCodeForSession(code).then(({ error }) => {
        if (error) {
          ...
          setStage("success");
          redirectTimer = setTimeout(() => router.replace("/dashboard/student"), 2800);
        }
      });
    }
    ...
    <>
      <style>{`
        ...
        .btn-primary:hover {
          ...
          box-shadow: 0 8px 24px rgb(var(--primary) / 0.38);
        }
        .btn-ghost {
          display: inline-flex; align-items: center; gap: 6px;
          padding: 11px 22px;
          ...
          {stage === "success" && (
            <>
              ...
              <p style={{ margin: "16px 0 0", fontSize: "12px", color: "rgb(var(--muted2))" }}>
                Not redirecting?{" "}
                <a href="/dashboard/student" style={{ color: "rgb(var(--primary))", fontWeight: 600, textDecoration: "none" }}>
                  Click here
                </a>
              </p>
            </>
