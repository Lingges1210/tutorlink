...
export default function CallbackPage() {
  ...
  const [errorMsg, setErrorMsg] = useState("This link may have expired or already been used.");

  useEffect(() => {
    // Check for hash error first (e.g. otp_expired)
    const hash = window.location.hash.slice(1);
    const hashParams = new URLSearchParams(hash);
    const hashError = hashParams.get("error");
    if (hashError) {
      const desc = hashParams.get("error_description")?.replace(/\+/g, " ") ?? "Link expired or invalid.";
      setErrorMsg(desc);
      setStage("error");
      return;
    }
    ...
    // Handle ?code= param (PKCE flow fallback)
    ...
    // Poll for session at intervals as fallback
