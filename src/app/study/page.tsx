...
/* Body bob up/down while walking */
@keyframes bodyBob {
...
/* Gentle idle sway (student standing) */
@keyframes idleSway {
...
/* Thinking dots for robot screen */
...
// ── AI Responses ───────────────────────────────────────────────────────────
const AI_RESPONSES = {
  stressed:   { text: "Take a breath — you've got this! 3 days is plenty if you're strategic. I'll build you a focused Study Plan with short daily sessions using active recall. No cramming, just smart studying.", cta: [{ label: "📅 Build My Study Plan", action: "/study/plan" }] },
  plan:       { text: "Smart move! A personalised Study Plan will keep you on track. I'll generate a daily schedule with goals, revision blocks, and rest days built in — tailored to your exam date.", cta: [{ label: "📅 Create Study Plan", action: "/study/plan" }] },
  flashcards: { text: "Active recall boosts retention by up to 50% vs re-reading! Upload your notes to Study Hub and I'll auto-generate flashcards and quizzes instantly — like having a tutor 24/7.", cta: [{ label: "📚 Open Study Hub", action: "/study/hub" }] },
  notes:      { text: "Your study pack is being built right now! Head to Study Hub — you'll find a summary, key concepts, flashcards, and a practice quiz all generated from your notes.", cta: [{ label: "📚 Go to Study Hub", action: "/study/hub" }] },
  hub:        { text: "Study Hub is your command centre. Upload lecture notes or a PDF and within seconds you'll have an AI-generated summary, key concepts, flashcards, and a quiz. Zero manual work.", cta: [{ label: "📚 Open Study Hub", action: "/study/hub" }, { label: "➕ Upload Notes", action: "/study/hub/upload" }] },
  default_a:  { text: "I'd start by uploading your notes to Study Hub for instant flashcards, then set up a Study Plan for a clear daily schedule all the way to exam day. Both tools work great together!", cta: [{ label: "📚 Study Hub", action: "/study/hub" }, { label: "📅 Study Plan", action: "/study/plan" }] },
  default_b:  { text: "Use Study Hub to turn your notes into flashcards and practice tests, then pair it with a Study Plan to stay organised. Together they cover both what to study and when to study it.", cta: [{ label: "📅 Create a Plan", action: "/study/plan" }] },
  default_c:  { text: "The most effective approach is active recall — testing yourself rather than re-reading. Upload to Study Hub, generate a quiz, repeat daily. Want me to also build you a revision schedule?", cta: [{ label: "📚 Upload & Study", action: "/study/hub/upload" }, { label: "📅 Build Schedule", action: "/study/plan" }] },
};
...
function FloatingParticles() {
  ...
  useEffect(() => {
    ...
    let animId: number;
    const particles: { x:number;y:number;vx:number;vy:number;r:number;alpha:number;pulse:number;pulseSpeed:number;shape:"circle"|"ring"|"dot"|"cross" }[] = [];
    const rockets: { x:number;y:number;angle:number;speed:number;size:number;alpha:number;trailPoints:{x:number;y:number}[];wobble:number;wobbleSpeed:number }[] = [];
    const astronauts: { cx:number;cy:number;orbitRx:number;orbitRy:number;phase:number;orbitSpeed:number;bobPhase:number;bobSpeed:number;size:number;alpha:number;rotation:number;rotSpeed:number }[] = [];
    ...
    for (let i=0;i<55;i++) {
      ...
      particles.push({x:Math.random()*window.innerWidth,y:Math.random()*window.innerHeight,vx:(Math.random()-0.5)*0.4,vy:(Math.random()-0.5)*0.4,r:Math.random()*3.5+1,alpha:Math.random()*0.3+0.07,pulse:Math.random()*Math.PI*2,pulseSpeed:0.01+Math.random()*0.02,shape:shapes[Math.floor(Math.random()*shapes.length)]});
    }
    function spawnRocket() {
      const side=Math.floor(Math.random()*4); let x=0,y=0,angle=0;
    ...
    for (const s of [{cx:0.08,cy:0.18},{cx:0.91,cy:0.25},{cx:0.85,cy:0.72},{cx:0.12,cy:0.78}]) {
      astronauts.push({cx:s.cx,cy:s.cy,orbitRx:0.032+Math.random()*0.02,orbitRy:0.018+Math.random()*0.015,phase:Math.random()*Math.PI*2,orbitSpeed:0.003+Math.random()*0.003,bobPhase:Math.random()*Math.PI*2,bobSpeed:0.007+Math.random()*0.005,size:28+Math.random()*14,alpha:0.28+Math.random()*0.2,rotation:Math.random()*Math.PI*2,rotSpeed:(Math.random()-0.5)*0.005});
    }

    function isDark(){return document.documentElement.classList.contains("dark");}
    function drawRocket(x:number,y:number,angle:number,size:number,alpha:number,col:string){ctx!.save();ctx!.translate(x,y);ctx!.rotate(angle+Math.PI/2);ctx!.globalAlpha=alpha;ctx!.beginPath();ctx!.moveTo(0,-size);ctx!.bezierCurveTo(size*0.42,-size*0.5,size*0.42,size*0.28,0,size*0.52);ctx!.bezierCurveTo(-size*0.42,size*0.28,-size*0.42,-size*0.5,0,-size);ctx!.fillStyle=`rgba(${col},${alpha*0.85})`;ctx!.fill();const fl=ctx!.createRadialGradient(0,size*0.62,0,0,size*1.0,size*0.55);fl.addColorStop(0,`rgba(255,215,60,${alpha*1.1})`);fl.addColorStop(0.4,`rgba(255,100,20,${alpha*0.7})`);fl.addColorStop(1,`rgba(255,40,0,0)`);ctx!.beginPath();ctx!.ellipse(0,size*0.82,size*0.24,size*0.52,0,0,Math.PI*2);ctx!.fillStyle=fl;ctx!.fill();ctx!.restore();}
    function drawAstronaut(x:number,y:number,size:number,alpha:number,rotation:number,col:string){ctx!.save();ctx!.translate(x,y);ctx!.rotate(rotation);ctx!.globalAlpha=alpha;const s=size;ctx!.beginPath();ctx!.ellipse(0,s*0.12,s*0.38,s*0.44,0,0,Math.PI*2);ctx!.fillStyle=`rgba(${col},${alpha*0.55})`;ctx!.fill();ctx!.beginPath();ctx!.arc(0,-s*0.35,s*0.3,0,Math.PI*2);ctx!.fillStyle=`rgba(${col},${alpha*0.45})`;ctx!.fill();ctx!.restore();}

    let rocketSpawnTimer=0;
    function draw(){
      if(!ctx||!canvas)return; ctx.clearRect(0,0,canvas.width,canvas.height);
      const dark=isDark(); const baseColor=dark?"200,210,255":"99,102,241";
      for(const p of particles){p.pulse+=p.pulseSpeed;const ga=p.alpha*(0.6+0.4*Math.sin(p.pulse));p.x+=p.vx;p.y+=p.vy;if(p.x<-20)p.x=canvas.width+20;if(p.x>canvas.width+20)p.x=-20;if(p.y<-20)p.y=canvas.height+20;if(p.y>canvas.height+20)p.y=-20;ctx.save();if(p.shape==="dot"){ctx.beginPath();ctx.arc(p.x,p.y,p.r*0.5,0,Math.PI*2);ctx.fillStyle=`rgba(${baseColor},${ga*1.2})`;ctx.fill();}else if(p.shape==="ring"){ctx.beginPath();ctx.arc(p.x,p.y,p.r*2.5,0,Math.PI*2);ctx.strokeStyle=`rgba(${baseColor},${ga*0.45})`;ctx.lineWidth=0.8;ctx.stroke();}else if(p.shape==="cross"){const ss=p.r*1.8;ctx.strokeStyle=`rgba(${baseColor},${ga*0.45})`;ctx.lineWidth=0.8;ctx.beginPath();ctx.moveTo(p.x-ss,p.y);ctx.lineTo(p.x+ss,p.y);ctx.stroke();ctx.beginPath();ctx.moveTo(p.x,p.y-ss);ctx.lineTo(p.x,p.y+ss);ctx.stroke();}else{const gr=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,p.r*3.5);gr.addColorStop(0,`rgba(${baseColor},${ga})`);gr.addColorStop(1,`rgba(${baseColor},0)`);ctx.beginPath();ctx.arc(p.x,p.y,p.r*3.5,0,Math.PI*2);ctx.fillStyle=gr;ctx.fill();}ctx.restore();}
      for(let i=0;i<particles.length;i++)for(let j=i+1;j<particles.length;j++){const dx=particles[i].x-particles[j].x,dy=particles[i].y-particles[j].y,dist=Math.sqrt(dx*dx+dy*dy);if(dist<110){ctx.beginPath();ctx.moveTo(particles[i].x,particles[i].y);ctx.lineTo(particles[j].x,particles[j].y);ctx.strokeStyle=`rgba(${baseColor},${(1-dist/110)*0.05})`;ctx.lineWidth=0.5;ctx.stroke();}}
      rocketSpawnTimer++;if(rocketSpawnTimer>300&&rockets.length<4){spawnRocket();rocketSpawnTimer=0;}
      for(let i=rockets.length-1;i>=0;i--){const r=rockets[i];r.wobble+=r.wobbleSpeed;const wa=r.angle+Math.sin(r.wobble)*0.05;r.x+=Math.cos(wa)*r.speed;r.y+=Math.sin(wa)*r.speed;r.trailPoints.unshift({x:r.x,y:r.y});if(r.trailPoints.length>60)r.trailPoints.pop();for(let tt=0;tt<r.trailPoints.length-1;tt++){const prog=1-tt/r.trailPoints.length;ctx.beginPath();ctx.moveTo(r.trailPoints[tt].x,r.trailPoints[tt].y);ctx.lineTo(r.trailPoints[tt+1].x,r.trailPoints[tt+1].y);ctx.strokeStyle=`rgba(${baseColor},${prog*r.alpha*0.28})`;ctx.lineWidth=prog*2.5;ctx.stroke();}drawRocket(r.x,r.y,wa,r.size,r.alpha,baseColor);const margin=150;if(r.x<-margin||r.x>canvas.width+margin||r.y<-margin||r.y>canvas.height+margin){rockets.splice(i,1);rocketSpawnTimer=350;}}
      for(const a of astronauts){a.phase+=a.orbitSpeed;a.bobPhase+=a.bobSpeed;a.rotation+=a.rotSpeed;const px=(a.cx+Math.cos(a.phase)*a.orbitRx)*canvas.width,py=(a.cy+Math.sin(a.phase)*a.orbitRy+Math.sin(a.bobPhase)*0.008)*canvas.height;drawAstronaut(px,py,a.size,a.alpha,a.rotation,baseColor);}
...
function AIChatWidget() {
  ...
  // Walk-in on mount: walk for 1s then stop
  useEffect(() => {
  ...
  const handleSend = useCallback((text?: string) => {
    ...
    setInputValue(""); // clear but DO NOT refocus
    ...
    setStudentWalking(true);  // student walks a bit when asking
    setRobotThinking(true);
    setIsTyping(true);

    // Student stops walking after ~700ms, remains idle
    ...
    setTimeout(() => {
      ...
      setMessages(prev => [...prev, { id: ++msgIdRef.current, role: "ai", text: resp.text, cta: resp.cta }]);
  ...
  return (
    <section className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card2)/0.45)] dark:bg-[rgb(var(--card2)/0.35)] backdrop-blur-sm overflow-hidden">
      ...
      <div className="relative px-6 pt-6 pb-4 border-b border-[rgb(var(--border))] bg-[rgb(var(--card)/0.22)] overflow-hidden">
        ...
        {/* Walking path — dotted floor line */}
            ...
            <div className="flex flex-col gap-1.5 max-w-[78%]">
              ...
              {msg.cta && msg.cta.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-0.5">
                  {msg.cta.map((c,i)=>(
                    <Link key={i} href={c.action}
                      className="inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-[11px] font-semibold border border-sky-400/30 bg-sky-500/10 text-sky-700 dark:text-sky-300 hover:bg-sky-500/20 hover:-translate-y-0.5 transition-all duration-150">
      ...
      {/* Input — no autoFocus, no refocus after send */}
...
function TabButton({ active, children, onClick }: { active:boolean;children:React.ReactNode;onClick:()=>void }) {
  return (
    <button type="button" onClick={onClick}
      className={["relative inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-semibold transition-all duration-200",
        active?"bg-[rgb(var(--fg))] text-[rgb(var(--bg))] shadow-[0_4px_20px_rgb(var(--shadow)/0.3)] scale-[1.02]"
              :"bg-transparent border border-[rgb(var(--border))] text-[rgb(var(--muted))] hover:text-[rgb(var(--fg))] hover:border-[rgb(var(--fg)/0.3)] hover:scale-[1.01]"].join(" ")}>
      {children}
    </button>
  );
}
...
// ── Step Card ──────────────────────────────────────────────────────────────
function StepCard({ num, title, desc, icon: Icon, accentClass, lineClass }: { num:number;title:string;desc:string;icon:any;accentClass:string;lineClass:string }) {
  return (
    <div className="group relative rounded-2xl border p-5 overflow-hidden border-[rgb(var(--border))] bg-[rgb(var(--card)/0.65)] dark:bg-[rgb(var(--card)/0.5)] backdrop-blur-sm transition-all duration-300 ease-out hover:-translate-y-2 hover:bg-[rgb(var(--card)/0.92)] dark:hover:bg-[rgb(var(--card)/0.85)] hover:shadow-[0_20px_56px_rgb(var(--shadow)/0.22)]">
      ...
      <div className="relative flex items-start gap-3">
        <div className={["h-9 w-9 shrink-0 rounded-xl inline-flex items-center justify-center bg-[rgb(var(--card2)/0.8)] border border-[rgb(var(--border))] transition-transform duration-300 group-hover:scale-110",accentClass].join(" ")}><Icon className="h-4 w-4"/></div>
...
// ── Quick Card ─────────────────────────────────────────────────────────────
function QuickCard({ title, desc, icon: Icon, href, tag, accentClass, tagBg, tagText }: { title:string;desc:string;icon:any;href:string;tag:string;accentClass:string;tagBg:string;tagText:string }) {
  return (
    <Link href={href} className="group relative rounded-2xl border p-5 overflow-hidden flex flex-col border-[rgb(var(--border))] bg-[rgb(var(--card)/0.65)] dark:bg-[rgb(var(--card)/0.5)] backdrop-blur-sm transition-all duration-300 ease-out hover:-translate-y-2 hover:bg-[rgb(var(--card)/0.92)] dark:hover:bg-[rgb(var(--card)/0.85)] hover:shadow-[0_20px_56px_rgb(var(--shadow)/0.22)]">
      ...
      <div className="relative flex items-start justify-between gap-3">
        <div className={["h-9 w-9 rounded-xl inline-flex items-center justify-center shrink-0 border border-[rgb(var(--border))] bg-[rgb(var(--card2)/0.8)] transition-all duration-300 group-hover:scale-110",accentClass].join(" ")}><Icon className="h-4 w-4"/></div>
      ...
      <div className="relative mt-5 inline-flex items-center gap-1.5 text-xs font-semibold text-[rgb(var(--primary))]">
        Get started<ArrowRight className="h-3.5 w-3.5 group-hover:translate-x-1.5 transition-transform duration-200"/>
      </div>
    </Link>
  );
}
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-2">
                <div className="inline-flex items-center gap-1.5 rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--card)/0.65)] dark:bg-[rgb(var(--card)/0.5)] backdrop-blur-sm p-1">
                  <TabButton active={tab==="hub"}  onClick={()=>setTab("hub")} ><BookOpen      className="h-3.5 w-3.5"/>Study Hub</TabButton>
                  <TabButton active={tab==="plan"} onClick={()=>setTab("plan")}><CalendarClock className="h-3.5 w-3.5"/>Study Plan</TabButton>
                </div>
                <span key={modeLabel} className="text-[11px] text-[rgb(var(--muted2))] pl-1" style={{ animation:"labelFadeIn 0.22s ease-out both" }}>— {modeLabel}</span>
              </div>
            </div>
          ...
          <section className="rounded-2xl border p-6 border-[rgb(var(--border))] bg-[rgb(var(--card2)/0.45)] dark:bg-[rgb(var(--card2)/0.35)] backdrop-blur-sm">
            <div className="flex items-center justify-between gap-3 pb-5 border-b border-[rgb(var(--border))]">
              <div key={`title-${tab}`} className="inline-flex items-center gap-2 text-sm font-semibold text-[rgb(var(--fg))]" style={{ animation:"labelFadeIn 0.22s ease-out both" }}>
                <div className="h-6 w-6 rounded-lg bg-[rgb(var(--fg))] flex items-center justify-center"><Wand2 className="h-3.5 w-3.5 text-[rgb(var(--bg))]"/></div>
                How {tab==="hub" ? "Study Hub" : "Study Plan"} Works
              </div>
              <div key={`meta-${tab}`} className="text-[11px] text-[rgb(var(--muted2))] font-medium" style={{ animation:"labelFadeIn 0.25s ease-out both" }}>{modeLabel} · 4 steps</div>
            </div>
            <div key={tab} className="mt-5 grid grid-cols-1 md:grid-cols-4 gap-3" style={{ animation:"stepFadeIn 0.3s ease-out both" }}>
              {howItWorks.map((s,i)=>(
                <div key={s.num} style={{ animation:`stepFadeIn 0.3s ease-out ${i*55}ms both` }}>
                  <StepCard {...s}/>
                </div>
              ))}
            </div>
