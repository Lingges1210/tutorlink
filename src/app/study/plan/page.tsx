...
import {
  ...
  ChevronRight, Clock, BarChart2, Lightbulb, Zap, Brain,
} from "lucide-react";
...
export default function StudyPlanPage() {
  ...
  async function toggleItem(item: PlanItem) {
    ...
    if (next === "DONE") { studypalReward("streak"); }
    try {
  ...
  return (
    <>
      <style>{`
        ...
        .spg-ghost { background:rgb(var(--card2)); border:1px solid rgb(var(--border)); color:rgb(var(--fg)); transition:all .15s; }
        .spg-ghost:hover:not(:disabled) { border-color:rgba(var(--primary),.4); color:rgb(var(--primary)); }
        .spg-ghost:disabled { opacity:.45; }
        .spg-cta { background:rgb(var(--primary)); color:#fff; border:none; transition:all .18s; }
        ...
        .spg-grad  { background:linear-gradient(135deg,rgb(var(--primary)),rgba(var(--primary),.55)); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        .spg-ai-banner { background:rgba(var(--primary),.05); border:1px solid rgba(var(--primary),.16); }
        .spg-tip { background:rgba(var(--primary),.06); border:1px solid rgba(var(--primary),.15); border-radius:10px; }
        input[type=range].spg-range { -webkit-appearance:none; appearance:none; width:100%; height:4px; border-radius:99px; background:rgb(var(--card2)); outline:none; cursor:pointer; }
        input[type=range].spg-range::-webkit-slider-thumb { -webkit-appearance:none; width:17px; height:17px; border-radius:50%; background:rgb(var(--primary)); cursor:pointer; border:2.5px solid rgb(var(--card)); box-shadow:0 1px 6px rgba(var(--primary),.35); transition:transform .15s; }
        input[type=range].spg-range:hover::-webkit-slider-thumb { transform:scale(1.18); }
        input[type=range].spg-range::-moz-range-thumb { width:17px; height:17px; border-radius:50%; background:rgb(var(--primary)); cursor:pointer; border:none; }
        .spg-input { background:rgb(var(--card2)); border:1px solid rgb(var(--border)); color:rgb(var(--fg)); outline:none; transition:border-color .15s,box-shadow .15s; font-family:inherit; }
        .spg-input::placeholder { color:rgb(var(--muted)); opacity:.6; }
        .spg-input:focus { border-color:rgba(var(--primary),.5); box-shadow:0 0 0 3px rgba(var(--primary),.1); }
        ...
        .spg-sec-nav { background:rgb(var(--card2)); border:1px solid rgb(var(--border)); border-radius:12px; padding:4px; display:flex; gap:2px; }
        .spg-sec-btn { flex:1; border:none; border-radius:9px; padding:8px 12px; font-size:12px; font-weight:600; cursor:pointer; transition:all .15s; font-family:inherit; display:flex; align-items:center; justify-content:center; gap:6px; }
        .spg-sec-btn-active { background:rgb(var(--card)); border:1.5px solid rgba(var(--primary),.5); color:rgb(var(--primary)); box-shadow:0 1px 4px rgba(0,0,0,.06); }
        .spg-sec-btn-inactive { background:transparent; border:1px solid transparent; color:rgb(var(--muted)); }
        .spg-sec-btn-inactive:hover { color:rgb(var(--fg)); }
        .spg-label { font-size:11px; font-weight:600; letter-spacing:.04em; text-transform:uppercase; color:rgb(var(--muted2)); margin-bottom:6px; }
        .spg-opt { border-radius:12px; padding:12px 14px; cursor:pointer; transition:all .15s; text-align:left; font-family:inherit; }
        .spg-opt-on { background:rgba(var(--primary),.08); border:2px solid rgb(var(--primary)); color:rgb(var(--primary)); box-shadow:0 0 0 3px rgba(var(--primary),.1); }
        .spg-opt-off { background:transparent; border:1.5px solid rgb(var(--border)); color:rgb(var(--fg)); }
        .spg-opt-off:hover { border-color:rgba(var(--primary),.4); }
        .spg-mascot-wrap { position:fixed; bottom:24px; right:28px; z-index:40; pointer-events:none; }
        .spg-mascot-float { animation:spgMascotFloat 3.8s ease-in-out infinite; position:relative; display:flex; flex-direction:column; align-items:center; }
        @keyframes spgMascotFloat { 0%,100%{transform:translateY(0px) rotate(-1deg)} 50%{transform:translateY(-10px) rotate(1deg)} }
        .spg-arm-l { transform-origin:22px 56px; animation:spgArmWobble 3.8s ease-in-out infinite; }
        @keyframes spgArmWobble { 0%,100%{transform:rotate(0deg)} 30%{transform:rotate(-8deg)} 60%{transform:rotate(4deg)} }
        .spg-blink { animation:spgBlink 4s ease-in-out infinite; opacity:0; }
        @keyframes spgBlink { 0%,92%,100%{opacity:0} 94%,98%{opacity:1} }
        .spg-sparkle1 { animation:spgSpark1 2.4s ease-in-out infinite; transform-origin:61.5px 18px; }
        @keyframes spgSpark1 { 0%,100%{opacity:.9;transform:scale(1) rotate(0deg)} 50%{opacity:.4;transform:scale(0.6) rotate(20deg)} }
        .spg-sparkle2 { animation:spgSpark2 3.1s ease-in-out infinite; transform-origin:11px 12px; }
        @keyframes spgSpark2 { 0%,100%{opacity:.9;transform:scale(1)} 40%{opacity:.2;transform:scale(0.5)} }
        .spg-bubble { background:rgb(var(--card)); border:1px solid rgb(var(--border)); border-radius:12px; padding:5px 10px; margin-bottom:6px; order:-1; box-shadow:0 4px 16px rgba(0,0,0,.1); position:relative; animation:spgBubblePop 4s ease-in-out infinite; pointer-events:none; }
        .spg-bubble::after { content:''; position:absolute; bottom:-6px; left:50%; transform:translateX(-50%); border-left:6px solid transparent; border-right:6px solid transparent; border-top:6px solid rgb(var(--card)); }
        .spg-bubble::before { content:''; position:absolute; bottom:-7px; left:50%; transform:translateX(-50%); border-left:7px solid transparent; border-right:7px solid transparent; border-top:7px solid rgb(var(--border)); }
        .spg-bubble-text { font-size:11px; font-weight:600; color:rgb(var(--fg)); white-space:nowrap; font-family:inherit; }
        @keyframes spgBubblePop { 0%,100%{opacity:1;transform:scale(1)} 45%,55%{opacity:0;transform:scale(0.85)} }
        ...
        .spg-num-input::-webkit-inner-spin-button,.spg-num-input::-webkit-outer-spin-button { -webkit-appearance:none; margin:0; }
        .spg-num-input { -moz-appearance:textfield; }
        .spg-subj-card { border:1px solid rgb(var(--border)); border-radius:16px; overflow:hidden; }
        .spg-subj-head { background:rgb(var(--card2)); border-bottom:1px solid rgb(var(--border)); padding:14px 16px; display:flex; align-items:center; justify-content:space-between; }
        .spg-level-track { height:6px; border-radius:99px; background:rgb(var(--card2)); border:1px solid rgb(var(--border)); overflow:hidden; }
        .spg-level-fill { height:100%; background:linear-gradient(90deg,rgba(var(--primary),.5),rgb(var(--primary))); border-radius:99px; transition:width .3s ease; }
      `}</style>
      ...
      <div className="pt-8 pb-20">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 sm:px-6">
          ...
          <header className="spg-up mb-6">
            <Link href="/study?tab=plan" className="inline-flex items-center gap-1.5 text-sm spg-muted hover:spg-acc-text transition-colors mb-3">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="text-sm spg-muted mt-1">Smart weekly schedule that adapts to your weak spots and keeps you on track every day.</p>
              </div>
              {step==="RESULT" && (
                <button type="button" onClick={()=>setStep("FORM")} className="spg-ghost inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-semibold shrink-0 mt-1">
                  <Wand2 className="h-3.5 w-3.5" /> Edit inputs
                </button>
              )}
            </div>
          </header>
          ...
          {error && (
            <div className="spg-fd rounded-2xl border border-red-500/25 bg-red-500/8 p-4 mb-6 text-sm text-red-500 flex items-start gap-3">
              ...
              <button type="button" onClick={()=>setError("")} className="hover:opacity-70 transition-opacity"><X className="h-4 w-4" /></button>
            </div>
          )}
          ...
          {step==="RESULT" && plan?.aiExplanation && (
            <div className="spg-ai-banner rounded-xl px-4 py-3 mb-4 spg-up d1 flex items-start gap-2.5">
              ...
              <p className="text-xs spg-muted leading-relaxed">
                <span className="font-semibold spg-acc-text">Why this plan? </span>{plan.aiExplanation}
              </p>
            </div>
          )}
            ...
            <section className="spg-card rounded-3xl p-6 shadow-sm mb-6 spg-up d1">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {todayPack.todays.length>0 && (
                  <span className="text-xs spg-muted flex items-center gap-1">
                    {todayPack.pending.length===0
                      ? <><CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />All done</>
              ...
              {todayPack.todays.length===0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {todayPack.todays.map((it,i)=>{
                    ...
                    return (
                      <div key={it.id} className={cx("rounded-2xl overflow-hidden spg-up",done?"spg-done":"spg-card2")} style={{animationDelay:`${i*40}ms`}}>
                        <div className={cx("h-[3px] w-full",TYPE_BAR[it.type]??TYPE_BAR.STUDY)} />
                        <button type="button" onClick={()=>toggleItem(it)} disabled={busyToggle===it.id} className="spg-task w-full text-left px-4 py-3.5">
                          <div className="flex items-start gap-3">
                            <div className={cx("mt-0.5 h-5 w-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all",done?"bg-emerald-500 border-emerald-500":"border-current/20")}>
                              {done && <CheckCircle2 className="h-3 w-3 text-white" />}
                            </div>
                            <div className="min-w-0 flex-1">
                              <div className="flex items-start justify-between gap-2">
                                <p className={cx("text-sm font-semibold spg-fg leading-snug",done&&"line-through opacity-35")}>{cleanTask(it.task)}</p>
                              ...
                              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                                <span className={cx("inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold",TYPE_PILL[it.type]??TYPE_PILL.STUDY)}>
                                  <Icon className="h-2.5 w-2.5" />{label}
                                </span>
                                {it.timeBlock && <span className="spg-card rounded-full px-2 py-0.5 text-[10px] spg-muted2">{it.timeBlock}</span>}
                              </div>
                            </div>
                          </div>
                        </button>
                        <div className="px-4 pb-3">
                          <button type="button" onClick={()=>setWhyId(it.id)} className="inline-flex items-center gap-1 text-[11px] font-semibold spg-acc-text hover:underline">
              ...
              <div className="spg-divider mt-5 pt-4 flex flex-wrap gap-2">
                <button type="button" onClick={()=>fetchPlan()} disabled={loading} className="spg-ghost inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-semibold">
                  <RefreshCcw className={cx("h-3.5 w-3.5",loading&&"spg-spin")} /> Refresh
                </button>
                <button type="button" onClick={rebalance} disabled={loading} className="spg-ghost inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-semibold">
                  <Wand2 className="h-3.5 w-3.5" /> Rebalance week
                </button>
                <button type="button" onClick={()=>setStep("FORM")} className="spg-cta ml-auto inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold">
                  ...
                  <div className="spg-sec-nav">
                    {FORM_SECTIONS.map(({id,label,icon:Icon})=>(
                      <button key={id} type="button" onClick={()=>setActiveSection(id)}
                        className={cx("spg-sec-btn",activeSection===id?"spg-sec-btn-active":"spg-sec-btn-inactive")}>
                  ...
                  <div className={activeSection===0?"spg-panel-active":"spg-panel"}>
                    <div className="space-y-5">
                      <div>
                        ...
                        <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="e.g., CALC Final Sprint" className="spg-input w-full rounded-xl px-3.5 py-2.5 text-sm" />
                      </div>
                      <div>
                        <p className="spg-label">Exam / deadline date</p>
                        <input type="date" value={examDate} onChange={e=>setExamDate(e.target.value)} className="spg-input w-full rounded-xl px-3.5 py-2.5 text-sm" style={{colorScheme:"inherit"}} />
                      </div>
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <p className="spg-label mb-0">Hours per week</p>
                          <span className="inline-block rounded-lg px-2.5 py-1 text-xs font-bold spg-acc-pill">{hoursPerWeek}h / week</span>
                        </div>
                        <input type="range" min={1} max={30} value={hoursPerWeek} onChange={e=>setHPW(+e.target.value)} className="spg-range w-full" />
                        <div className="flex justify-between text-[10px] spg-muted mt-1.5"><span>1h</span><span>30h</span></div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {([
                            ...
                            <button key={s} type="button" onClick={()=>setStyle(s)} className={cx("spg-opt",style===s?"spg-opt-on":"spg-opt-off")}>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {([
                            ...
                            <button key={t} type="button" onClick={()=>setPT(t)} className={cx("spg-opt text-center",preferredTime===t?"spg-opt-on":"spg-opt-off")}>
                      ...
                      <button type="button" onClick={()=>setActiveSection(1)} className="spg-cta w-full inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold">
                    ...
                    <div className="space-y-5">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {DAY_KEYS.map(d=>(
                            <button key={d} type="button" onClick={()=>setDays(p=>p.includes(d)?p.filter(x=>x!==d):[...p,d])}
                              className={cx("rounded-full px-3.5 py-1.5 text-xs font-semibold transition-all",days.includes(d)?"spg-day-on":"spg-day-off")}>
                              {DAY_LABEL[d]}
                            </button>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                          {DAY_KEYS.map(d=>(
                            <div key={d} className={cx("rounded-xl px-3 py-2.5 flex items-center justify-between gap-2 transition-all",days.includes(d)?"spg-card2":"opacity-40 spg-card2")}>
                              <span className={cx("text-xs font-semibold",days.includes(d)?"spg-acc-text":"spg-muted")}>{DAY_LABEL[d]}</span>
                              <input type="number" min={0} max={8} step={0.5} value={hoursByDay[d]}
                      ...
                      <div className="flex gap-2 pt-2">
                        <button type="button" onClick={()=>setActiveSection(0)} className="spg-ghost flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold">Back</button>
                        <button type="button" onClick={()=>setActiveSection(2)} className="spg-cta flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold">
                  ...
                  <div className={activeSection===2?"spg-panel-active":"spg-panel"}>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <button type="button" onClick={()=>setSubjects(p=>[...p,{name:"",level0to10:5,weakTopics:[],weakInput:""}])}
                          className="spg-ghost inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-semibold">+ Add subject</button>
                      </div>
                          ...
                          <div key={idx} className="spg-subj-card">
                            <div className="spg-subj-head">
                              <div className="flex items-center gap-2">
                                <span className="h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-bold spg-acc-pill">{idx+1}</span>
                                <p className="text-xs font-semibold spg-fg">{s.name||`Subject ${idx+1}`}</p>
                              </div>
                              {subjects.length>1 && (
                                <button type="button" onClick={()=>setSubjects(p=>p.filter((_,i)=>i!==idx))}
                            ...
                            <div className="p-4 space-y-4">
                              ...
                              <div>
                                ...
                                <div className="spg-level-track mb-2"><div className="spg-level-fill" style={{width:`${s.level0to10*10}%`}} /></div>
                                ...
                                <div className="flex justify-between text-[10px] spg-muted mt-1"><span>Beginner</span><span>Expert</span></div>
                              </div>
                              <div>
                                <p className="text-xs font-medium spg-muted mb-1.5">Weak topics <span className="opacity-60">(comma separated)</span></p>
                                <input value={s.weakInput}
                      ...
                      <div className="flex gap-2 pt-2">
                        <button type="button" onClick={()=>setActiveSection(1)} className="spg-ghost rounded-xl px-4 py-3 text-sm font-semibold">Back</button>
                        <button type="button" onClick={generatePlan} disabled={loading} className="spg-cta flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold">
                          {loading
              ...
              <aside className="space-y-4 spg-up d2">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-2.5">
                        <div className="h-6 w-6 rounded-lg flex items-center justify-center spg-acc-pill shrink-0"><Icon className="h-3 w-3" /></div>
                ...
                <div className="spg-card rounded-2xl p-5 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {FORM_SECTIONS.map(({id,label,icon:Icon})=>(
                      <button key={id} type="button" onClick={()=>setActiveSection(id)}
                        className={cx("w-full flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-xs font-semibold transition-all text-left",activeSection===id?"spg-opt-on":"spg-opt-off")}>
                ...
                <div className="spg-tip p-4">
                  <p className="text-xs font-bold spg-acc-text mb-1.5 flex items-center gap-1.5">
          ...
          {step==="RESULT" && plan && (
            <div className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {([
                  {label:"Plan",       value:plan.title||"My Study Plan", sub:`${prettyDate(plan.startDate)} → ${prettyDate(plan.endDate)}`, extra:`${prefLabel(plan.preferredTime??preferredTime)}`, Icon:BookOpen},
                  {label:"Progress",   value:`${progress.pct}%`,          sub:`${progress.done} of ${progress.total} tasks`, Icon:BarChart2},
                  {label:"Focus area", value:progress.weakSpot??"—",      sub:"Most pending tasks here", Icon:Target},
                ] as any[]).map((c,i)=>(
                  <div key={i} className="spg-card rounded-2xl p-4 shadow-sm spg-up" style={{animationDelay:`${i*60}ms`}}>
                    <div className="flex items-center justify-between mb-2.5">
                      <p className="text-[11px] font-medium spg-muted2">{c.label}</p>
                      <div className="h-6 w-6 rounded-lg flex items-center justify-center spg-acc-pill"><c.Icon className="h-3 w-3" /></div>
                    </div>
              ...
              <section className="spg-card rounded-3xl shadow-sm spg-up d2 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b" style={{borderColor:"rgb(var(--border))"}}>
                  ...
                  <div className="flex items-center gap-3 text-[11px] spg-muted flex-wrap">
                    {[{cls:"spg-bar-indigo",label:"Study"},{cls:"spg-bar-sky",label:"Practice"},{cls:"spg-bar-amber",label:"Revision"},{cls:"spg-bar-purple",label:"Get help"}].map(({cls,label})=>(
                      <span key={label} className="flex items-center gap-1">
                        <span className={cx("inline-block h-2 w-2 rounded-full",cls)} />{label}
                      </span>
                    ))}
                  </div>
                </div>
                ...
                <div className="divide-y" style={{borderColor:"rgb(var(--border))"}}>
                  {grouped.map((day,di)=>{
                    ...
                    return (
                      <div key={day.date} className={cx("spg-up",isToday&&"spg-today-col")} style={{animationDelay:`${di*25}ms`}}>
                        <div className={cx("flex items-center gap-4 px-6 py-3",isToday?"spg-today-hdr":"")}>
                          <div className="w-20 shrink-0">
                            <p className={cx("text-xs font-bold",isToday?"spg-acc-text":"spg-fg")}>{prettyDate(day.date).split(",")[0]}</p>
                            <p className="text-[10px] spg-muted">{prettyDate(day.date).split(",").slice(1).join("").trim()}</p>
                          </div>
                            ...
                            {day.items.map(it=>{
                              ...
                              return (
                                <div key={it.id} className="flex flex-col gap-1 min-w-0">
                                  <button type="button" onClick={()=>toggleItem(it)} disabled={busyToggle===it.id}
                                    className={cx("spg-task flex items-center gap-2 rounded-xl px-3 py-2 text-left",done?"spg-done":"spg-card2")}>
                                    <span className={cx("h-2 w-2 rounded-full shrink-0 mt-px",TYPE_BAR[it.type]??TYPE_BAR.STUDY)} />
                                    <div className={cx("h-4 w-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-all",done?"bg-emerald-500 border-emerald-500":"border-current/20")}>
                                      {done && <CheckCircle2 className="h-2.5 w-2.5 text-white" />}
                                    </div>
                                    <div className="min-w-0">
                                      <p className={cx("text-xs font-semibold spg-fg whitespace-nowrap",done&&"line-through opacity-35")}>{cleanTask(it.task)}</p>
                                      <p className="text-[10px] spg-muted mt-0.5 whitespace-nowrap">
                                        {it.subjectName} · {it.durationMin}m{it.timeBlock && <span className="opacity-60"> · {it.timeBlock}</span>}
                                      </p>
                                    </div>
                                  </button>
                                  <button type="button" onClick={()=>setWhyId(it.id)} className="ml-2 inline-flex items-center gap-0.5 text-[10px] font-semibold spg-acc-text hover:underline">
                          ...
                          <div className="shrink-0 text-right hidden sm:block">
                            ...
                            <div className={cx("text-[10px] font-semibold px-2.5 py-1 rounded-full inline-flex items-center gap-1",
                              allDone?"bg-emerald-500/10 text-emerald-600 border border-emerald-500/20":"spg-muted2 spg-card2")}>
                              {allDone ? <><CheckCircle2 className="h-3 w-3" />Done</> : `${doneCount}/${day.items.length}`}
                            </div>
                            <p className="text-[10px] spg-muted mt-1">{totalMin}m total</p>
                            {isToday && <span className="inline-block rounded-full px-2 py-0.5 text-[9px] font-bold spg-acc-pill mt-1">TODAY</span>}
                          </div>
                        </div>
                      </div>
                    );
          ...
          {step==="RESULT" && !plan && (
            <div className="spg-card rounded-3xl p-12 text-center shadow-sm spg-up">
              ...
              <p className="mt-1.5 text-sm spg-muted max-w-xs mx-auto leading-relaxed">Create your first plan to see your weekly schedule here.</p>
              <button type="button" onClick={()=>setStep("FORM")} className="spg-cta mt-5 inline-flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold">
      ...
      <div className="spg-mascot-wrap" aria-hidden="true">
        <div className="spg-mascot-float">
          ...
          <div className="spg-bubble">
            <p className="spg-bubble-text flex items-center gap-1.5">
              You got this!
              <BookOpen style={{display:"inline",width:11,height:11,color:"rgb(var(--primary))"}} />
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="spg-modal spg-pop relative w-full max-w-sm rounded-3xl overflow-hidden">
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-3 mb-4">
                ...
                <button type="button" onClick={()=>setWhyId(null)} className="spg-ghost h-8 w-8 rounded-xl flex items-center justify-center shrink-0">
              ...
              <div className="spg-inset p-4 mb-4">
                ...
                {whyItem.timeBlock && (
                  <span className="inline-block rounded-full px-2.5 py-0.5 text-xs font-medium spg-acc-pill mb-2">{whyItem.timeBlock}</span>
                )}
              ...
              {plan?.aiExplanation && (
                <p className="text-xs spg-muted leading-relaxed mb-4">
                  <span className="font-semibold spg-fg">Plan context: </span>{plan.aiExplanation}
                </p>
              )}
              <button type="button" onClick={()=>setWhyId(null)} className="spg-ghost w-full rounded-xl py-2.5 text-sm font-semibold">Got it</button>
            </div>
          </div>
        </div>
      )}
