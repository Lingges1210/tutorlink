              ...
              <nav className="p-1.5 space-y-0.5">
                <button type="button" onClick={() => setActiveSubjectId("")}
                  className={`w-full flex items-center justify-between rounded-xl px-2.5 py-2 text-xs font-medium transition-colors
                    ${activeSubjectId === ""
                      ? "bg-[rgb(var(--primary))/0.12] text-[rgb(var(--primary))] border border-[rgb(var(--primary))/0.25]"
                ...
                {subjects.map((s) => (
                  <button key={s.id} type="button" onClick={() => setActiveSubjectId(s.id)}
                    className={`w-full flex items-center justify-between rounded-xl px-2.5 py-2 text-xs font-medium transition-colors
                    ...
                    <span className="flex items-center gap-2 min-w-0">
                      <div className="h-1.5 w-1.5 rounded-full bg-[rgb(var(--primary))/0.6] shrink-0" />
            ...
            <div className="flex items-center justify-between gap-4 mb-4">
              ...
              <div className="relative">
                ...
                <input value={search} onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search…"
                  className="h-9 w-48 rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] pl-8 pr-3 text-sm text-[rgb(var(--fg))] placeholder:text-[rgb(var(--muted2))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))/0.25] focus:border-[rgb(var(--primary))/0.4] transition-all" />
              </div>
            </div>

            {err && (
              <div className="mb-3 rounded-xl border border-red-400/25 bg-red-50 dark:bg-red-500/8 px-3 py-2.5 text-xs text-red-600 dark:text-red-400">{err}</div>
            )}

            {loading ? (
              <div className="space-y-2">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="h-[68px] rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] animate-pulse" style={{ opacity: 1 - i * 0.25 }} />
                ))}
              </div>
            ) : filtered.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-[rgb(var(--border))] bg-[rgb(var(--card))] p-10 text-center">
                <div className="inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] mb-4">
                  <BookOpen className="h-5 w-5 text-[rgb(var(--muted2))]" />
                </div>
                ...
                {!search && (
                  <Link href="/study/hub/upload"
                    className="mt-4 inline-flex items-center gap-1.5 h-9 px-4 rounded-xl text-sm font-semibold text-white"
                    style={{ background: "linear-gradient(135deg,rgb(139,92,246),rgb(217,70,239))" }}>
              ...
              <div className="space-y-2">
                {filtered.map((m) => (
                  <div key={m.id}
                    className="relative rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] group hover:border-[rgb(var(--primary))/0.3] hover:shadow-sm transition-all duration-150">
                    <Link href={`/study/hub/${m.id}`} className="flex items-center gap-3 px-4 py-3.5 pr-12">
                      <div className="h-9 w-9 shrink-0 rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] flex items-center justify-center group-hover:border-[rgb(var(--primary))/0.3] transition-colors">
                        <BookOpen className="h-4 w-4 text-[rgb(var(--muted2))] group-hover:text-[rgb(var(--primary))] transition-colors" />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-2 flex-wrap">
                          ...
                          {m.studySubjectId && (() => {
                            ...
                            return subj ? (
                              <span className="text-[10px] border border-[rgb(var(--primary))/0.25] bg-[rgb(var(--primary))/0.08] text-[rgb(var(--primary))] rounded-full px-2 py-0.5">
                                {subj.name}
                              </span>
                            ) : null;
                          })()}
          ...
          <div className="relative w-full max-w-sm rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[rgb(var(--border))]">
              ...
              <button onClick={() => setRenameOpen(false)}
                className="h-7 w-7 rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card2))] inline-flex items-center justify-center text-[rgb(var(--muted2))] hover:text-[rgb(var(--fg))] transition-colors">
            ...
            <div className="p-5 space-y-3">
              <input value={renameValue} onChange={(e) => setRenameValue(e.target.value)}
                ...
                className="w-full rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] px-3.5 py-2.5 text-sm text-[rgb(var(--fg))] placeholder:text-[rgb(var(--muted2))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--primary))/0.25] focus:border-[rgb(var(--primary))/0.4] transition-all"
                placeholder="Material title" autoFocus />
              <div className="flex justify-end gap-2">
                <button onClick={() => setRenameOpen(false)}
                  className="h-9 px-4 rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--card2))] text-sm font-medium text-[rgb(var(--fg))] hover:bg-[rgb(var(--card))] transition-colors">
                  Cancel
                </button>
      ...
      {deleteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[rgb(var(--border))]">
              <div className="flex items-center gap-2">
                <div className="h-6 w-6 rounded-lg border border-amber-400/30 bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center">
            ...
            <div className="p-5 space-y-3">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={doDelete}
                  className="h-9 px-4 rounded-xl bg-red-600 hover:bg-red-500 text-sm font-semibold text-white transition-colors shadow-sm">
                  Delete
                </button>
              </div>
            </div>
