...
export default function NotificationsBellClient({
  ...
  const baseSessionsPath = useCallback((viewer?: ViewerHint | null) => {
  ...
  const buildFocusHref = useCallback((sessionId?: string, viewer?: ViewerHint | null, page?: number) => {
  ...
  const pickHref = useCallback((n: NotiItem) => {
  ...
  const deleteOne = useCallback(async (n: NotiItem) => {
  ...
  }, [deletingId, items, unread]);

  const openNotification = useCallback(async (n: NotiItem) => {
    setErr(null);

    if (!n.readAt) {
      setItems((p) =>
        p.map((x) => (x.id === n.id ? { ...x, readAt: new Date().toISOString() } : x))
      );
  ...
  const panel = useMemo(() => (
    ...
    >
      <div className="flex items-center justify-between gap-3 px-4 py-3.5 border-b border-[rgb(var(--border))] bg-[rgb(var(--card2)/0.6)]">
        <div className="flex items-center gap-2.5">
          <div className="h-7 w-7 rounded-xl grid place-items-center bg-[rgb(var(--primary)/0.12)] ring-1 ring-[rgb(var(--primary)/0.2)]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <motion.button
            ...
            className="rounded-xl border p-1.5 transition-all duration-150 border-[rgb(var(--border))] bg-[rgb(var(--card2))] text-[rgb(var(--muted))] hover:text-[rgb(var(--fg))] hover:bg-[rgb(var(--card))] hover:shadow-sm"
          >
            <X size={13} />
          </motion.button>
        </div>
        ...
        {loading ? (
          <div className="space-y-2.5 p-1">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="rounded-2xl border border-[rgb(var(--border))] p-3.5 bg-[rgb(var(--card2))] animate-pulse"
          ...
          <motion.div
            ...
            className="rounded-2xl border border-rose-500/30 bg-rose-500/8 p-3.5 text-sm text-rose-600 dark:text-rose-400 flex items-center gap-2.5"
          >
          ...
          >
            <div className="h-12 w-12 rounded-2xl grid place-items-center bg-[rgb(var(--border)/0.4)] border border-[rgb(var(--border))]">
          ...
          <>
            ...
            {items.length > 3 && (
              <div className="mt-2">
                <motion.button
                  ...
                  className="w-full rounded-xl border px-3 py-2.5 text-xs font-semibold border-[rgb(var(--border))] bg-[rgb(var(--card2))] text-[rgb(var(--fg))] hover:bg-[rgb(var(--card))] hover:shadow-sm flex items-center justify-center gap-2 transition-all duration-150"
                >
                  <span>{showAll ? "Show less" : `Show all ${items.length} notifications`}</span>
            ...
            <div className="mt-2.5 pt-2.5 border-t border-[rgb(var(--border)/0.6)]">
              <motion.button
                ...
                className="w-full rounded-xl px-3 py-2 text-xs font-semibold text-rose-500 dark:text-rose-400 hover:bg-rose-500/8 transition-all duration-150 flex items-center justify-center gap-1.5"
              >
                <Trash2 size={12} />
                <span>Clear all</span>
              </motion.button>
            ...
            <AnimatePresence>
              {confirmClearOpen && (
                ...
                >
                  <motion.div
                    className="w-full max-w-[300px] rounded-2xl border p-4 border-[rgb(var(--border))] bg-[rgb(var(--card))] shadow-[0_20px_60px_rgb(0,0,0,0.25)]"
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="mt-0.5 h-9 w-9 rounded-xl grid place-items-center bg-rose-500/12 border border-rose-500/25 shrink-0">
                    ...
                    <div className="mt-4 flex items-center justify-end gap-2">
                      <button
                        ...
                        className="rounded-xl border px-3 py-1.5 text-xs font-semibold border-[rgb(var(--border))] bg-[rgb(var(--card2))] text-[rgb(var(--fg))] hover:bg-[rgb(var(--card))]"
                      >
                        Cancel
                      </button>
                      <button
