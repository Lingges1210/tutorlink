...
function selectedToAvailability(selected: Set<string>, stepMin: number): AvailabilityState {
  ...
  return days.map((day) => {
    ...
    if (!blocks.length) {
      return { day, off: true, slots: [{ start: "", end: "" }] };
    }
...
export default function AvailabilityHeatmap({ value, onChange, stepMin = 30 }: Props) {
  ...
  const jumpTo = useCallback((target: number) => {
    ...
    window.setTimeout(() => {
      setStagedStartMin(target);
      setPhase("in");
    }, 220);

    window.setTimeout(() => {
      setWindowStartMin(target);
  ...
  const startSlide = useCallback((dir: SlideDir) => {
    const next =
      dir === "next"
  ...
  const times = useMemo(
    () => buildTimes(windowStart, windowEnd, stepMin),
  ...
  function onMouseEnter(day: DayKey, t: number) {
    if (drag.current.active) applyBlock(day, t);
  }
              ...
              <div style={{ display: "inline-block" }}>
                <div className="grid items-end" style={{ ...gridStyle, marginBottom: 12 }}>
                  ...
                  {days.map((d) => {
                    ...
                    const full = hits === times.length;
                    const partial = hits > 0 && !full;
                ...
                <div
                  style={{
                    ...
                    transition:
                      "transform 500ms cubic-bezier(.22,1,.36,1), opacity 500ms cubic-bezier(.22,1,.36,1), filter 500ms cubic-bezier(.22,1,.36,1)",
                  ...
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
                    <span
                      style={{
                        ...
                        borderRadius: 4,
                        background: OFF_BG,
                        boxShadow: `inset 0 0 0 1px ${OFF_BD}`,
                      }}
                    />
                    Unavailable
                  </span>
